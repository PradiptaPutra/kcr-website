...
import { Desktop, Factory, CompassTool, ArrowRight } from '@phosphor-icons/react';
...
const About: React.FC = () => {
  ...
  return (
    <div className="bg-[#F5F5F0] min-h-screen pt-20 md:pt-24 pb-20 md:pb-32 selection:bg-[#1A1C19] selection:text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-20 lg:gap-32 items-center">
          <motion.div {...fadeInUp} className="lg:col-span-6 relative">
            <div className="aspect-[4/5] bg-[#1A1C19] overflow-hidden rounded-[4px] shadow-premium">
              <img src="/assets/images/overview/MODERNA_SERIES_cropped.jpg" className="w-full h-full object-cover opacity-90 transition-transform duration-[3s] hover:scale-110" alt="KCR Manufacturing" loading="lazy" />
            </div>
          ...
          <motion.div {...fadeInUp} transition={{ delay: 0.2 }} className="lg:col-span-6">
            ...
            <div className="space-y-10">
              <p className="framer-body !text-lg leading-relaxed text-[#1A1C19]/80">{kcrData.company.description}</p>
              <p className="framer-body leading-relaxed">{kcrData.company.introduction}</p>
              <div className="pt-10 border-t border-[#1A1C19]/5">
                <p className="font-serif text-[22px] italic text-brand leading-relaxed">"{kcrData.company.specialization}"</p>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {/* Vision */}
      <section className="bg-[#1A1C19] text-white py-32 md:py-56 relative overflow-hidden">
        {/* Background Decor */}
        ...
        <div className="framer-container relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start mb-40">
            <motion.div {...fadeInUp}>
              ...
              <p className="framer-body !text-white/50 !text-lg max-w-md leading-relaxed">{kcrData.company.vision}</p>
            </motion.div>
            <motion.div {...fadeInUp} className="aspect-video bg-black overflow-hidden rounded-[4px] border border-white/10 lg:ml-auto w-full shadow-2xl">
               <img src="/assets/images/workstation/WORKSTATION_MODERNA_cropped.jpg" className="w-full h-full object-cover opacity-40 grayscale hover:opacity-60 hover:grayscale-0 transition-all duration-[2s]" alt="Vision" loading="lazy" />
            </motion.div>
          </div>
          ...
          <motion.div {...fadeInUp} className="border-t border-white/10 pt-24">
            ...
            <div className="space-y-0">
              {kcrData.services.map((service, idx) => (
                <div key={idx} className="flex flex-col md:flex-row gap-8 py-14 border-b border-white/5 group hover:bg-white/[0.02] px-10 transition-all duration-700">
                  ...
                  <div className="md:w-2/6 framer-body !text-white/30 hidden md:flex items-center gap-4 group-hover:text-white/60 transition-colors">
                    {service.description}
                  </div>
                </div>
              ))}
      ...
      {/* Capabilities */}
      <section className="py-32 md:py-56 framer-container">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-20">
           {[ 
             { icon: <Factory weight="light" size={48} />, t: 'Manufaktur Mandiri', d: 'Fasilitas produksi sendiri memastikan kontrol kualitas penuh and ketepatan waktu pengiriman.' },
             { icon: <CompassTool weight="light" size={48} />, t: 'CNC Precision', d: 'Teknologi CNC standar Eropa menjamin akurasi milimeter and konsistensi pada setiap unit produk.' },
             { icon: <Desktop weight="light" size={48} />, t: 'Custom Design', d: 'Kemampuan adaptasi desain and material untuk memenuhi kebutuhan spesifik and identitas brand Anda.' }
           ].map((i, idx) => (
             <motion.div 
               key={idx}
               {...fadeInUp}
